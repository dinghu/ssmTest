...
public class JwtUtil {
    ...
    private static final String JWT_SECRET = "7786df7fc3a34e26a61c034d5ec8245d0000000000000000";

    public static String getUserNameFromJwtoken(String token) throws Exception {
        Claims claims = parseJWT(token);
        return (String) claims.get("user_name");
    }

    public static String getUserIdFromJwtoken(String token) throws Exception {
        Claims claims = parseJWT(token);
        return (String) claims.get("user_id");
    }

    public static String getUserNickNameFromJwtoken(String token) throws Exception {
        Claims claims = parseJWT(token);
        return (String) claims.get("nick_name");
    }
...
//        SecretKey key = new SecretKeySpec(encodedKey, 0, encodedKey.length, signatureAlgorithm.getJcaName());// 根据给定的字节数组使用AES加密算法构造一个密钥，使用 encodedKey中的始于且包含 0 到前 leng 个字节这是当然是所有。（后面的文章中马上回推出讲解Java加密和解密的一些算法）
        return key;
    }
     ...
     * @throws Exception
     */
    public static String createJWT(String user_id, String user_name, String subject) throws Exception {
