...
public class UserController {
    ...
    public Response login(String email, String password, HttpServletResponse response) {
        ...
        Integer userId = userService.selectUserIdByEmailAndPassword(email, md5Pwd);
        User user = userService.selectUserInfoByUserId(userId);
        String jwtToken = "";
        //jwt
        try {
            jwtToken = JwtUtil.createJWT(String.valueOf(user.getUserId()), user.getUsername(), user.getUsername());
            System.out.println("jwtToken: " + jwtToken);
            String uid = JwtUtil.getUserIdFromJwtoken(jwtToken);
